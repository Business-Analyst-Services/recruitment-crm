...
| **11** Security & retention | Protect data; destroy or de-identify when no longer needed | Local SQLite, no third-party processors. **Erase candidate** hard-deletes all linked records | **Authentication and roles (not built yet)**, encryption at rest, backups, audit log, automated retention review |
...
**Do not load real candidate data until authentication, HTTPS hosting and backups are in place.**
